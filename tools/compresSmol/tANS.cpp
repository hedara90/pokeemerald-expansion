...
std::vector<EncodeCol> createEncodingTable(std::vector<DecodeCol> decodeTable, std::vector<unsigned char> symbols)
{
    ...
    for (DecodeCol dCol : decodeTable)
    {
        ...
        {
            //printf("%i: %hhu %hhu\n", symbolIndex, encodeHelpers[symbolIndex].symbol, currSymbol);
            symbolIndex++;
        }
...
std::vector<unsigned char> findSymbols(std::vector<unsigned char> input)
{
    ...
    //  Setting all symbols to exist instead of only the found ones
    /*
    for (int i = 0; i < input.size(); i++)
    {
        bool foundSym = false;
...
bool areVectorsEqual(std::vector<unsigned char> input, std::vector<unsigned char> output)
{
    if (input.size() != output.size())
        return false;
    /*
    std::string inputString = getStringFromUCvec(input);
    std::string outputString = getStringFromUCvec(output);
    for (int i = 0; i < input.size()/64; i++)
    {
        for (int k = 0; k < 64; k++)
            std::cout << inputString[i*64+k];
        std::cout << "\n";
        for (int k = 0; k < 64; k++)
            std::cout << outputString[i*64+k];
        std::cout << "\n";
    }
    */
    for (int i = 0; i < input.size(); i++)
