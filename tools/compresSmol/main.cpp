...
int main(int argc, char *argv[])
{
    ...
    std::string output;
    int numThreads = 1;
    InputSettings settings(true, true, true, true);
        ...
        case ANALYZE:
            ...
            if (argc > 8)
            {
                ...
                if (setting3.compare("true") == 0)
                    settings.canDeltaLO = true;
                else if (setting3.compare("false") == 0)
                    settings.canDeltaLO = false;
                else
                    fprintf(stderr, "Unrecognized setting3 \"%s\", defaulting to \"true\"\n", setting1.c_str());
                if (setting4.compare("true") == 0)
                    settings.canDeltaSyms = true;
                else if (setting4.compare("false") == 0)
                    settings.canDeltaSyms = false;
                else
                    fprintf(stderr, "Unrecognized setting4 \"%s\", defaulting to \"true\"\n", setting1.c_str());
            }
            break;
        case WRITE:
            if (argc > 3)
    ...
    if (option == DECODE)
    {
        if (std::filesystem::exists(input))
        {
            ...
            std::vector<unsigned char> image4bpp = decodeCompression(&inData);
            std::ofstream fileOut(output.c_str(), std::ios::out | std::ios::binary);
            std::copy(image4bpp.begin(), image4bpp.end(), std::ostreambuf_iterator<char>(fileOut));
            fileOut.close();
        }
        else
        {
