...
static u16 RenderText(struct TextPrinter *textPrinter)
{
    ...
    case RENDER_STATE_HANDLE_CHAR:
        if ((JOY_HELD(A_BUTTON | B_BUTTON) || JOY_HELD(R_BUTTON)) && subStruct->hasPrintBeenSpedUp)
            textPrinter->delayCounter = 0;

        if (textPrinter->delayCounter && textPrinter->textSpeed)
        {
            textPrinter->delayCounter--;
            if (gTextFlags.canABSpeedUpPrint && (JOY_HELD(A_BUTTON | B_BUTTON) || JOY_HELD(R_BUTTON)))
            {
        ...
        if (JOY_HELD(A_BUTTON | B_BUTTON) || JOY_HELD(R_BUTTON))
            repeats = 3;
