...
DOUBLE_BATTLE_TEST("Sheer Force only boosts the damage of moves it's supposed to boost")
{
    s16 damage1, damage2;
    u32 move = 0;
    for (u32 j = 1; j < MOVES_COUNT; j++)
        if (gMovesInfo[j].category != DAMAGE_CATEGORY_STATUS
         && j != MOVE_PSYWAVE       //  Just skip Psywve
         && j != MOVE_PRESENT       //  And Present...
         && j != MOVE_MAGNITUDE     //  And Magnitude...
         && j != MOVE_ERUPTION      //  And Eruption...
         && j != MOVE_WATER_SPOUT
         && j != MOVE_GYRO_BALL
         && j != MOVE_SYNCHRONOISE
         && j != MOVE_ELECTRO_BALL
         && j != MOVE_ROUND
         && j != MOVE_BELCH
         && j != MOVE_HYPERSPACE_FURY
         && j != MOVE_BURN_UP
         && j != MOVE_SHELL_TRAP
         && j != MOVE_BOLT_BEAK
         && j != MOVE_FISHIOUS_REND
         && j != MOVE_AURA_WHEEL
         && j != MOVE_STEEL_ROLLER
         && j != MOVE_DRAGON_ENERGY
         && j != MOVE_DOUBLE_SHOCK
         && j != MOVE_COMEUPPANCE
         && j != MOVE_UPPER_HAND    //  Bugged?
         && j != MOVE_SALT_CURE     //  Shoulg get boosted according to list
         && j != MOVE_EERIE_SPELL   // Should get boosted, it's explicitly made to be boosted
         && j != MOVE_ZIPPY_ZAP     //  Should not get boosted?
         && j != MOVE_GLITZY_GLOW     //  Should not get boosted?
         && j != MOVE_PAYBACK)
            PARAMETRIZE { move = j; }
    GIVEN {
